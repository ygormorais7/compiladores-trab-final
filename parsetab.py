...
_lr_productions = [
  ...
  ('program -> PROGRAM ID LBRACE decl_list stmt_list RBRACE','program',6,'p_program','parser.py',23),
  ('decl_list -> decl decl_list','decl_list',2,'p_decl_list','parser.py',28),
  ('decl_list -> empty','decl_list',1,'p_decl_list','parser.py',29),
  ('decl -> type ID ASSIGN literal SEMICOLON','decl',5,'p_decl','parser.py',38),
  ('decl -> type id_list SEMICOLON','decl',3,'p_decl','parser.py',39),
  ...
  ('type -> CONST','type',1,'p_type','parser.py',68),
  ('id_list -> ID id_tail','id_list',2,'p_id_list','parser.py',74),
  ('id_tail -> COLON ID id_tail','id_tail',3,'p_id_tail','parser.py',83),
  ('id_tail -> empty','id_tail',1,'p_id_tail','parser.py',84),
  ('stmt_list -> stmt stmt_list','stmt_list',2,'p_stmt_list','parser.py',93),
  ('stmt_list -> empty','stmt_list',1,'p_stmt_list','parser.py',94),
  ('stmt -> assign_stmt','stmt',1,'p_stmt','parser.py',103),
  ('stmt -> print_stmt','stmt',1,'p_stmt','parser.py',104),
  ('stmt -> input_stmt','stmt',1,'p_stmt','parser.py',105),
  ('stmt -> if_stmt','stmt',1,'p_stmt','parser.py',106),
  ('stmt -> while_stmt','stmt',1,'p_stmt','parser.py',107),
  ('assign_stmt -> ID ASSIGN exp SEMICOLON','assign_stmt',4,'p_assign_stmt','parser.py',113),
  ('print_stmt -> PRINT LPAREN exp_list RPAREN SEMICOLON','print_stmt',5,'p_print_stmt','parser.py',141),
  ('input_stmt -> INPUT LPAREN id_list RPAREN SEMICOLON','input_stmt',5,'p_input_stmt','parser.py',147),
  ('if_stmt -> IF LPAREN exp RPAREN LBRACE stmt_list RBRACE else_part','if_stmt',8,'p_if_stmt','parser.py',155),
  ('else_part -> ELSE LBRACE stmt_list RBRACE','else_part',4,'p_else_part','parser.py',171),
  ('else_part -> empty','else_part',1,'p_else_part','parser.py',172),
  ('while_stmt -> WHILE LPAREN exp RPAREN LBRACE stmt_list RBRACE','while_stmt',7,'p_while_stmt','parser.py',181),
  ('exp -> exp_relational','exp',1,'p_exp','parser.py',195),
  ('exp_relational -> exp_arithmetic','exp_relational',1,'p_exp_relational','parser.py',201),
  ('exp_relational -> exp_arithmetic GT exp_arithmetic','exp_relational',3,'p_exp_relational','parser.py',202),
  ('exp_relational -> exp_arithmetic LT exp_arithmetic','exp_relational',3,'p_exp_relational','parser.py',203),
  ('exp_relational -> exp_arithmetic GEQ exp_arithmetic','exp_relational',3,'p_exp_relational','parser.py',204),
  ('exp_relational -> exp_arithmetic LEQ exp_arithmetic','exp_relational',3,'p_exp_relational','parser.py',205),
  ('exp_relational -> exp_arithmetic EQ exp_arithmetic','exp_relational',3,'p_exp_relational','parser.py',206),
  ('exp_relational -> exp_arithmetic NEQ exp_arithmetic','exp_relational',3,'p_exp_relational','parser.py',207),
  ('exp_arithmetic -> exp_arithmetic PLUS term','exp_arithmetic',3,'p_exp_arithmetic','parser.py',216),
  ('exp_arithmetic -> exp_arithmetic MINUS term','exp_arithmetic',3,'p_exp_arithmetic','parser.py',217),
  ('exp_arithmetic -> term','exp_arithmetic',1,'p_exp_arithmetic','parser.py',218),
  ('term -> term TIMES factor','term',3,'p_term','parser.py',240),
  ('term -> term DIVIDE factor','term',3,'p_term','parser.py',241),
  ('term -> factor','term',1,'p_term','parser.py',242),
  ('term -> unary','term',1,'p_term','parser.py',243),
  ('unary -> NOT factor','unary',2,'p_unary','parser.py',265),
  ('unary -> MINUS factor','unary',2,'p_unary','parser.py',266),
  ('BOOL_CONST -> TRUE','BOOL_CONST',1,'p_bool_const','parser.py',285),
  ('BOOL_CONST -> FALSE','BOOL_CONST',1,'p_bool_const','parser.py',286),
  ('factor -> ID','factor',1,'p_factor','parser.py',292),
  ('factor -> INTEGER_CONST','factor',1,'p_factor','parser.py',293),
  ('factor -> FLOAT_CONST','factor',1,'p_factor','parser.py',294),
  ('factor -> STRING_CONST','factor',1,'p_factor','parser.py',295),
  ('factor -> BOOL_CONST','factor',1,'p_factor','parser.py',296),
  ('factor -> LPAREN exp RPAREN','factor',3,'p_factor','parser.py',297),
  ('empty -> <empty>','empty',0,'p_empty','parser.py',308),
  ('literal -> INTEGER_CONST','literal',1,'p_literal','parser.py',314),
  ('literal -> FLOAT_CONST','literal',1,'p_literal','parser.py',315),
  ('literal -> STRING_CONST','literal',1,'p_literal','parser.py',316),
  ('literal -> BOOL_CONST','literal',1,'p_literal','parser.py',317),
  ('exp_list -> exp exp_tail','exp_list',2,'p_exp_list','parser.py',323),
  ('exp_tail -> COLON exp exp_tail','exp_tail',3,'p_exp_tail','parser.py',332),
  ('exp_tail -> empty','exp_tail',1,'p_exp_tail','parser.py',333),
]
